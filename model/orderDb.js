...
const getOrderDb = async (id) => {
  try {
    let [results] = await pool.query("SELECT * FROM orders WHERE orderId=?", [
      id,
    ]);
    return results;
  } catch (error) {
...
const getOrderByUserDb = async (id) => {
  try {
    let [results] = await pool.query("SELECT * FROM orders WHERE userId=?", [
      id,
    ]);
    return results;
  } catch (error) {}
...
const addOrderDb = async (userId, itemId, bookingDate, bookingTime, totalPrice) => {
  ...
  console.log("Parameters:", { userId, itemId, bookingDate, bookingTime, totalPrice }); // Log the parameters
  try {
    await pool.query(
...
const updateOrderDb = async (
  ...
  try {
    await pool.query(
      "UPDATE  orders SET userId=?, itemId=?, bookingDate=?, bookingTime=?, totalPrice=? WHERE orderId=?",
      [userId, itemId, bookingDate, bookingTime, totalPrice, id]
    );
  } catch (error) {
